...
const escapeCsv = (value: unknown) => `"${text(value).replace(/"/g, '""')}"`;
...
function downloadCsv(rows: ProblemRecord[]) {
  const lines = [COLUMNS.map(escapeCsv).join(','), ...rows.map((row) => KEYS.map((key) => escapeCsv(row[key])).join(','))];
...
function downloadExcel(rows: ProblemRecord[]) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>table{border-collapse:collapse;font-family:Arial;font-size:10pt}th{background:#183b63;color:white;font-weight:bold;border:1px solid #b8c2cc;padding:7px}td{border:1px solid #d7dde4;padding:6px;vertical-align:top}tr:nth-child(even){background:#f4f7fa}</style></head><body><table><thead><tr>${header}</tr></thead><tbody>${body}</tbody></table></body></html>`;
  downloadBlob(`\uFEFF${html}`, 'application/vnd.ms-excel;charset=utf-8', `dynatrace-problem-alert-dump-${new Date().toISOString().slice(0, 10)}.xls`);
}
...
function dqlDuration(range: string) { return ['1h', '6h', '24h', '7d', '30d'].includes(range) ? range : '24h'; }
function buildProblemQuery(range: string, status: string, severity: string, zone: string) {
  ...
  let query = `fetch dt.davis.problems, from:now()-${dqlDuration(range)}, to:now() | filter ${filters.join(' and ')}`;
  if (zone !== 'All Management Zones') {
    const escaped = zone.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    query += ` | expand affected_entity_ids | lookup sourceField:affected_entity_ids, lookupField:id, [fetch dt.entity.host | filter managementZones == "${escaped}"], fields:{zoneHost.id=id} | filter isNotNull(zoneHost.id)`;
  }
  return `${query} | sort event.start desc | limit 1000`;
}
function severityLabel(value: unknown) { const n = Number(value); return Number.isFinite(n) ? `Level ${n}` : text(value) || '—'; }

function createModal() {
  const modal = document.createElement('div'); modal.className = 'alert-dump-modal';
  modal.innerHTML = `<div class="alert-dump-backdrop"></div><section class="alert-dump-card" role="dialog" aria-modal="true"><header class="alert-dump-header"><div><span class="alert-dump-eyebrow">LIVE DYNATRACE</span><h2>Dynatrace Alert Dump</h2><p>Download Davis Problem records for investigation, reporting and RCA.</p></div><button class="alert-dump-close" aria-label="Close">×</button></header><div class="alert-dump-controls"><label>Time range<select class="alert-dump-range"><option value="1h">Last 1 hour</option><option value="6h">Last 6 hours</option><option value="24h" selected>Last 24 hours</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option></select></label><label>Status<select class="alert-dump-status"><option value="ALL">All</option><option value="ACTIVE">Active</option><option value="CLOSED">Closed</option></select></label><label>Severity<select class="alert-dump-severity"><option value="ALL">All severities</option><option value="1">Level 1 — Critical</option><option value="2">Level 2</option><option value="3">Level 3</option><option value="4">Level 4</option><option value="5">Level 5 — Info</option></select></label><button class="alert-dump-load">Load problems</button></div><div class="alert-dump-scope"><span>Scope: <strong class="alert-dump-zone">All Management Zones</strong></span><span class="alert-dump-count">0 problems</span></div><div class="alert-dump-message"></div><div class="alert-dump-table-wrap"><table class="alert-dump-table"><thead><tr><th>Problem ID</th><th>Title</th><th>Status</th><th>Severity</th><th>Category</th><th>Started</th><th>Duration</th><th>Affected entities</th></tr></thead><tbody><tr><td colspan="8" class="alert-dump-empty">Loading live Davis problems…</td></tr></tbody></table></div><footer class="alert-dump-footer"><span>Up to 1,000 unique problems · duplicates excluded</span><div><button class="alert-dump-csv" disabled>Download CSV</button><button class="alert-dump-xls" disabled>Download Excel</button></div></footer></section>`;
  return modal;
}
...
function openAlertDump() {
  ...
  const zone = readTopbarValue(0, 'All Management Zones');
  const rangeSelect = modal.querySelector<HTMLSelectElement>('.alert-dump-range')!; rangeSelect.value = readTopbarValue(1, '24h');
  modal.querySelector<HTMLElement>('.alert-dump-zone')!.textContent = zone;
  ...
  const render = () => { tbody.innerHTML = rows.length ? rows.map((row) => { const status = text(row['event.status']); const started = text(row['event.start']); const ended = text(row['event.end']); const start = new Date(started); const end = ended ? new Date(ended) : null; const duration = !Number.isNaN(start.getTime()) ? `${Math.max(0, ((end && !Number.isNaN(end.getTime()) ? end.getTime() : Date.now()) - start.getTime()) / 3600000).toFixed(1)} h` : '—'; const names = text(row.affected_entity_names); return `<tr><td><strong>${text(row.display_id)}</strong></td><td>${text(row['event.name'])}</td><td class="alert-dump-status-${status.toLowerCase()}">${status}</td><td>${severityLabel(row['event.severity'])}</td><td>${text(row['event.category'])}</td><td>${Number.isNaN(start.getTime()) ? started : start.toLocaleString()}</td><td>${duration}</td><td>${names || text(row.affected_entity_ids)}</td></tr>`; }).join('') : '<tr><td colspan="8" class="alert-dump-empty">No problems matched the selected filters.</td></tr>'; modal.querySelector<HTMLElement>('.alert-dump-count')!.textContent = `${rows.length} problem${rows.length === 1 ? '' : 's'}`; csvButton.disabled = !rows.length; xlsButton.disabled = !rows.length; };
  const setMessage = (value: string) => { message.textContent = value; message.classList.toggle('show', Boolean(value)); };
  const load = async () => { loadButton.disabled = true; loadButton.textContent = 'Loading…'; setMessage(''); tbody.innerHTML = '<tr><td colspan="8" class="alert-dump-empty">Reading Davis problems from Grail…</td></tr>'; try { const selectedZone = readTopbarValue(0, 'All Management Zones'); const query = buildProblemQuery(rangeSelect.value, modal.querySelector<HTMLSelectElement>('.alert-dump-status')!.value, modal.querySelector<HTMLSelectElement>('.alert-dump-severity')!.value, selectedZone); rows = await executeDql(query); render(); } catch (error) { rows = []; render(); setMessage(error instanceof Error ? error.message : 'Unable to load Dynatrace problems.'); } finally { loadButton.disabled = false; loadButton.textContent = 'Load problems'; } };
  modal.querySelector<HTMLButtonElement>('.alert-dump-close')!.onclick = () => modal.remove(); modal.querySelector<HTMLElement>('.alert-dump-backdrop')!.onclick = () => modal.remove(); loadButton.onclick = () => void load(); csvButton.onclick = () => downloadCsv(rows); xlsButton.onclick = () => downloadExcel(rows); void load();
}
