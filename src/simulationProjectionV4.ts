...
function series(v: unknown): Array<number | null> {
  ...
  if (v && typeof v === 'object') {
    const o = v as AnyRecord;
    return series(o.values ?? o.data ?? o.value);
  }
  const n = scalar(v);
  return n === null ? [] : [n];
}
...
function hostKey(v: unknown): string {
  ...
  if (v && typeof v === 'object') {
    const o = v as AnyRecord;
    return hostKey(o.value ?? o.id ?? o.data ?? o.values);
  }
...
function timestamp(v: unknown): number | null {
  ...
  if (v && typeof v === 'object') {
    const o = v as AnyRecord;
    return timestamp(o.value ?? o.start ?? o.from);
  }
  return null;
}
...
function intervalMs(record: AnyRecord): number {
  ...
  if (typeof raw === 'string') {
    ...
    if (m) return Number(m[1]) * ({ ms: 1, s: 1000, m: 60000, h: 3600000, d: 86400000 }[m[2].toLowerCase()] ?? 3600000);
  }
  return 3600000;
}
...
function timeframeStart(record: AnyRecord): number | null {
  ...
  if (tf && typeof tf === 'object') {
    const o = tf as AnyRecord;
    return timestamp(o.start ?? o.from ?? o.begin);
  }
...
function pointSeries(record: AnyRecord, value: unknown) {
  ...
  if (start === null) return [] as Array<{ ts: number; value: number }>;
  const step = intervalMs(record);
  return values.flatMap((value, index) => value === null ? [] : [{ ts: start + index * step, value }]);
}
...
async function dql(query: string): Promise<AnyRecord[]> {
  try {
    ...
    console.error('[DYNATRACE][simulation DQL]', { query, error });
    throw error;
  }
}
...
function traffic(panel: HTMLElement) {
  ...
  return {
    current: Number(text.match(/Current traffic[^0-9]*([0-9.]+)\s*req\/min/i)?.[1] || 0),
    simulated: Number(text.match(/Simulated traffic[^0-9]*([0-9.]+)\s*req\/min/i)?.[1] || 0),
  };
}
...
function fit(xs: number[], ys: number[], current: number): Model {
  ...
  const denominator = pairs.reduce((s, [x]) => s + (x - mx) ** 2, 0);
  if (!denominator) return { slope: 0, intercept: 0, r2: 0, current, samples: pairs.length, fallback: true };
  const slope = pairs.reduce((s, [x, y]) => s + (x - mx) * (y - my), 0) / denominator;
  const intercept = my - slope * mx;
  const total = pairs.reduce((s, [, y]) => s + (y - my) ** 2, 0);
  const residual = pairs.reduce((s, [x, y]) => s + (y - (intercept + slope * x)) ** 2, 0);
  return { slope, intercept, r2: total ? Math.max(0, 1 - residual / total) : 0, current, samples: pairs.length, fallback: false };
}

function alignedPairs(resourceRecord: AnyRecord, resource: unknown, trafficRecord: AnyRecord, traffic: unknown): Array<[number, number]> {
  const r = series(resource);
  const t = series(traffic).map((v) => v === null ? null : v / 60);
  const count = Math.min(r.length, t.length);
  const direct: Array<[number, number]> = [];
  for (let i = 0; i < count; i += 1) {
    const x = t[i];
    const y = r[i];
    if (x !== null && y !== null && Number.isFinite(x) && Number.isFinite(y)) direct.push([x, y]);
  }
  if (direct.length >= 8) return direct;

  // Both queries use the same 30-day, 1-hour bucket definition. If a tenant
  // returns different aligned timeframes, fall back to timestamp matching.
  const rp = pointSeries(resourceRecord, resource);
  const tp = pointSeries(trafficRecord, traffic).map((p) => ({ ...p, value: p.value / 60 }));
  ...
  for (const a of rp) {
    ...
    for (const b of tp) {
      const delta = Math.abs(a.ts - b.ts);
      if (delta <= tolerance && delta < distance) { best = b; distance = delta; }
    }
    if (best) out.push([best.value, a.value]);
  }
  return out;
}
...
async function load(panel: HTMLElement) {
  ...
  try {
    ...
    const ids = hosts.map((record) => hostKey(record.id)).filter(Boolean);
    ...
    const trafficByHost = new Map<string, AnyRecord>();
    requestSeries.forEach((record) => trafficByHost.set(hostKey(record['dt.entity.host']), record));
    const models: Record<'cpu' | 'memory' | 'disk', Model> = {
      cpu: { r2: 0, slope: 0, intercept: 0, current: 0, samples: 0, fallback: true },
      memory: { r2: 0, slope: 0, intercept: 0, current: 0, samples: 0, fallback: true },
      disk: { r2: 0, slope: 0, intercept: 0, current: 0, samples: 0, fallback: true },
    };
    for (const key of ['cpu', 'memory', 'disk'] as const) {
      const xs: number[] = [];
      const ys: number[] = [];
      resources.forEach((record) => {
        const trafficRecord = trafficByHost.get(hostKey(record['dt.entity.host']));
        if (!trafficRecord) return;
        const resourceValues = series(record[key]);
        const trafficValues = series(trafficRecord.requests);
        const lastResource = [...resourceValues].reverse().find((v) => v !== null && Number.isFinite(v as number));
        const pairs = alignedPairs(record, record[key], trafficRecord, trafficRecord.requests);
        pairs.forEach(([x, y]) => {
          if (Number.isFinite(x) && Number.isFinite(y) && y >= 0 && y <= 100) { xs.push(x); ys.push(y); }
        });
        if (lastResource !== undefined && lastResource !== null && Number.isFinite(lastResource)) {
          models[key].current += Number(lastResource);
        }
      });
      const hostCount = resources.filter((record) => trafficByHost.has(hostKey(record['dt.entity.host']))).length;
      if (hostCount) models[key].current /= hostCount;
    ...
    warning.textContent = 'Live resource observations could not be aligned for this scope. No fabricated trend is shown.';
