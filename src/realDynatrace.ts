...
async function getHostSeries(): Promise<SeriesRecord[]> {
  const run = async (alias: 'cpuSeries'|'memorySeries'|'diskSeries', metric: string) => {
    ...
    try { return await executeDql<SeriesRecord>(`timeseries ${alias}=avg(${metric}), by:{dt.entity.host}, interval:1h, from:-24h | fieldsAdd ${current}=arrayLast(${alias}) | fields dt.entity.host, ${alias}, ${current}, timeframe, interval`); }
    catch { return []; }
  };
...
async function getNetworkSeries(): Promise<Map<string, Record<string, unknown>>> {
  try {
    const records = await executeDql<Record<string, unknown>>(`timeseries rx=avg(dt.host.net.nic.bytes_rx), tx=avg(dt.host.net.nic.bytes_tx), by:{dt.entity.host}, interval:1h, from:-24h`);
...
async function getApplicationThroughputSeries(): Promise<Map<string, ThroughputRecord>> {
  try {
    const records = await executeDql<ThroughputRecord>(`fetch spans | filter request.is_root_span == true | filter isNotNull(dt.entity.host) and isNotNull(dt.entity.service) | makeTimeseries throughputSeries=count(), by:{dt.entity.host}, interval:1m, from:-24h | fieldsAdd throughputCurrent=arrayLast(throughputSeries) | fields dt.entity.host, throughputSeries, throughputCurrent, timeframe, interval`);
...
export async function getHosts(managementZone?: string): Promise<Host[]> {
  const [entities, seriesRecords, networkByHost, throughputByHost] = await Promise.all([getHostEntities(managementZone), getHostSeries(), getNetworkSeries(), getApplicationThroughputSeries()]);
  ...
  return entities.map((entity) => {
    ...
    const cpu = numbers(series?.cpuSeries), memory = numbers(series?.memorySeries), disk = numbers(series?.diskSeries), rx = numbers(network?.rx), tx = numbers(network?.tx), appThroughput = numbers(throughput?.throughputSeries);
    ...
    const start = startMs(series?.timeframe ?? throughput?.timeframe); const step = intervalMs(series?.interval ?? throughput?.interval);
