...
import { askCapacityDavis, suggestedCapacityQuestions } from './interactiveCapacityAi';
...
export function installInteractiveCapacityAiV47() {
  ...
  root.innerHTML = `
    <div class="axis-davis-ai-head">
      <div><span class="eyebrow">Davis Intelligence</span><h2>Ask Davis about capacity</h2><p>Ask a question about the currently selected Axis Bank Management Zone. Davis uses live Dynatrace telemetry and available forecast context.</p></div>
      <span class="axis-davis-ai-badge">Interactive AI</span>
    </div>
  ...
  const ask = async () => {
    ...
    status.textContent = 'Davis is assessing the live capacity context…';
    answer.hidden = true;
    try {
      ...
      status.textContent = `Analysing ${hosts.length} live host${hosts.length === 1 ? '' : 's'} in ${zone}…`;
      ...
      const text = await askCapacityDavis(question, { managementZone: zone, timeRange: '24h', forecastHorizon: 30, hosts, forecasts });
      answer.innerHTML = esc(text).replace(/\n/g, '<br/>');
      answer.hidden = false;
      status.textContent = `Davis assessment · ${new Date().toLocaleTimeString()}`;
    } catch (error) {
