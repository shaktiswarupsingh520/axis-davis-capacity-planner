...
function createRcaPdf(problem:Row,problemId:string,answer:string,history:Row[]){
  ...
  table(doc,M,y,[70,85,85,65,80,134],['Problem ID','Started','Ended','Duration','Status','Root-cause entity'],history.slice(0,10).map(x=>[asText(x.display_id),fmtDate(x['event.start']),fmtDate(x['event.end']),formatMinutes(rowDurationMinutes(x)),asText(x['event.status'])||'-',asText(x['root_cause.smartscape_entity'])||'-']),21);y+=260;
