...
export function LineChart({ series: seriesProp = [], values, forecast = [], lower = [], upper = [], threshold, labels = [], yAxisLabel = 'Value', xAxisLabel = 'Time', unit = '' }: LineChartProps) {
  ...
  const max = hasData ? Math.max(unit === '%' ? 100 : 1, ...all) : 100;
  ...
  const midLabel = labels[Math.floor(labels.length / 2)] ?? 'Midpoint';
  const endLabel = labels[labels.length - 1] ?? 'Now';
  return <div className="chart-wrap labeled-chart">
    <div className="chart-title-row"><strong>{yAxisLabel}</strong>{unit && <span>Unit: {unit}</span>}</div>
    {!hasData ? <div className="chart-no-data"><strong>No usable trend data returned</strong><span>The selected scope/timeframe did not return numeric points for this metric.</span></div> : <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${yAxisLabel} over ${xAxisLabel}`}>
      ...
      {[0,25,50,75,100].map((tick) => { const value = max * tick / 100; const yy = y(value); return <g key={tick}><line x1={left} x2={xEnd} y1={yy} y2={yy} className="chart-grid"/><text x="7" y={yy + 4} className="chart-axis-label">{formatValue(value, unit)}</text></g>; })}
      {threshold !== undefined && <><line x1={left} x2={xEnd} y1={y(threshold)} y2={y(threshold)} className="threshold-line"/><text x={xEnd - 118} y={y(threshold) - 8} className="threshold-label">Capacity threshold {formatValue(threshold, unit)}</text></>}
      ...
      {actualForecast.length > 0 && <text x={left + split + 8} y={height - 25} className="chart-axis-label">Forecast ({actualForecast.length} points)</text>}
