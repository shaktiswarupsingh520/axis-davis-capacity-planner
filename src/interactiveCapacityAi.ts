...
function compactContext(context: CapacityAiContext) {
  const hosts = context.hosts.map((h) => {
  ...
  const forecasts = context.forecasts.map(({ host, forecast }) => ({
    host: host.name,
...
function buildPrompt(question: string, context: CapacityAiContext) {
  const data = JSON.stringify(compactContext(context));
...
Answer directly and practically. For numerical questions, show the relevant values and units. For risk questions, name the affected hosts when available. For recommendations, explain why the action is appropriate and distinguish observed facts from forecast/simulation results. If a simulation is supplied, use its exact traffic growth and projected values; do not substitute a default scenario. Keep the response under 500 words. Use concise headings and bullets when useful.`;
...
export async function askCapacityDavis(question: string, context: CapacityAiContext): Promise<string> {
  ...
  const response = await publicClient.recommenderConversation({
    body: {
      text: buildPrompt(q, context),
      context: [{ type: 'instruction', value: 'You are answering a live capacity-planning question. Use only the supplied Dynatrace context. Do not expose internal reasoning or claim access to data not supplied.' }],
    },
  });
...
export const suggestedCapacityQuestions = [
  ...
  'Summarize the capacity outlook for the next 30 days.',
];
