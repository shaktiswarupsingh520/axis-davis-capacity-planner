...
function HostDetail({ host, onBack }: { host: Host; onBack: () => void }) { const [metric, setMetric] = useState<MetricKey>('cpu'); const latest = host.telemetry.at(-1)!; const series = host.telemetry.map((point) => metric === 'network' ? (point.networkRx + point.networkTx) / 2 : point[metric]); return <div className="content"><button className="back-button" onClick={onBack}>← Back to inventory</button><PageTitle eyebrow="Live host detail" title={host.name}><StatusBadge value={host.profile}/></PageTitle><div className="detail-meta"><span><Server size={15}/> {host.environment}</span><span><Database size={15}/> {host.application}</span><span><Activity size={15}/> {host.id}</span></div><section className="metric-strip"><MetricCard label="CPU" value={`${Math.round(latest.cpu)}%`} detail="Current" icon={<Cpu/>}/><MetricCard label="Memory" value={`${Math.round(latest.memory)}%`} detail="Current" icon={<Database/>} tone="teal"/><MetricCard label="Disk" value={`${Math.round(latest.disk)}%`} detail="Current" icon={<HardDrive/>} tone="amber"/><MetricCard label="Network RX / TX" value={`${Math.round(latest.networkRx)}% / ${Math.round(latest.networkTx)}%`} detail="Current" icon={<Activity/>} tone="green"/></section><section className="panel chart-panel"><div className="panel-heading"><div><span className="eyebrow">24-hour live telemetry</span><h2>{metricLabels[metric]}</h2></div><select value={metric} onChange={(event) => setMetric(event.target.value as MetricKey)}><option value="cpu">CPU</option><option value="memory">Memory</option><option value="disk">Disk</option><option value="network">Network</option></select></div><LineChart values={series} threshold={metric === 'disk' ? 85 : 80}/></section></div>; }
